...
// tests specifics of DecodeAddress
// using test vectors from the zcash repository

func TestZcashAddressDecodeAddress(t *testing.T) {
	for i, v := range tAddrData() {
		testSrc, textSrcError := hex.DecodeString(v.taddrBytesHex)
		if textSrcError != nil {
			fmt.Println(textSrcError, i)
		}

		decodetaddr, decodetaddrerror := DecodeAddress(v.taddr, Mainnet())
		decodetexAddr, decodetexAddrError := DecodeAddress(v.texAddr, Mainnet())

		// check for errors
		if decodetaddrerror != nil {
			t.Errorf("%s,%d", decodetaddrerror, i)
		}
		if decodetexAddrError != nil {
			t.Errorf("%s,%d", decodetexAddrError, i)
		}
		if decodetaddr.P2pkh == nil {
			t.Errorf("%s", errors.New("p2pkh address not correctly decoded"))
		}
		if decodetexAddr.Tex == nil {
			t.Errorf("%s", errors.New("tex address not correctly decoded"))
		}
		if !bytes.Equal(testSrc, decodetaddr.P2pkh[:]) || !bytes.Equal(testSrc, decodetexAddr.Tex[:]) {
			t.Errorf("%s", errors.New("decoded addresses do not match test data"))
		}
	}
}
...
func TestZcashAddressDecodeUnifiedAddress(t *testing.T) {
	for i, k := range uAddrData() {
		...
		if zaddrError != nil {
			t.Errorf("%s%d", errors.New("error parsing unified address"), i)
		}

		if zaddr.Unified == nil {
			t.Errorf("%d%s", i, errors.New("did not decode as a unified address"))
		}
		if zaddr.P2pkh != nil || zaddr.P2sh != nil || zaddr.Sapling != nil {
			t.Errorf("%s%d", errors.New("only the unified address field should be non-nil"), i)
		}
	}
}
...
func TestUnifiedAddress(t *testing.T) {
	...
	for i, k := range uAddrData() {
		...
		decoded, e := unifiedaddress.DecodeUnified(k.UnifiedAddr, "u")
		...
		if decoded.Sapling != nil {
			if !bytes.Equal(decoded.Sapling[:], k.SaplingRawAddr) {
				t.Errorf("%s", errors.New("decoded sapling receiver does not match test data"))
			}
		}
		if decoded.Orchard != nil {
			if !bytes.Equal(decoded.Orchard[:], k.OrchardRawAddr) {
				t.Errorf("%s", errors.New("decoded orchard receiver does not match test data"))
			}
		}

		if decoded.Unknown != nil {
			for key, val := range decoded.Unknown {
				if k.UnknownTypecode != int64(key) {
					t.Errorf("%s", errors.New("decoded unknown receiver typecode does not match test data"))
				}
				if !bytes.Equal(val[:], k.UnknownBytes) {
					t.Errorf("%s", errors.New("decoded unknown receiver bytes do not match test data"))
				}
			}
		}

		encoded, encodeError := unifiedaddress.EncodeUnified(decoded, "u")
		if encodeError != nil {
			t.Errorf("%s", encodeError)
		}

		if k.UnifiedAddr != encoded {
			t.Errorf("%s", errors.New("unified address decoding failed"))
			fmt.Println("Unified address decoding failed")
		}
	}
}

type TAddrTestVector struct {
	taddr         string
	taddrBytesHex string
	texAddr       string
}

// data taken from known zcash test vectors
func tAddrData() []TAddrTestVector {
	return []TAddrTestVector{
		{
		...
		{
			taddr:         "t1aqnebXhA45WpgQHLiXTPU1Kk6rp8vVDDr",
			taddrBytesHex: "ba2230b41fdc81714328231f40ab73feb52645a4",
			texAddr:       "tex1hg3rpdqlmjqhzsegyv05p2mnl66jv3dykth955",
		},
		{
			taddr:         "t1UG6FVxexmJRFXG4gvEmSF9HSTwHMFaSDT",
			taddrBytesHex: "71f1fc6fd69370f23611536b3b64e7df1cebef69",
			texAddr:       "tex1w8clcm7kjdc0yds32d4nke88muwwhmmfunhkhd",
		},
	}
}

type UAddrTestVector struct {
	P2PKHBytes      []byte
	P2SHBytes       []byte
	SaplingRawAddr  []byte
	OrchardRawAddr  []byte
	UnknownTypecode int64
	UnknownBytes    []byte
	UnifiedAddr     string
}

// data taken from known zcash test vectors
func uAddrData() []UAddrTestVector {
	testVectors := []UAddrTestVector{
		{
			P2PKHBytes: []byte{
			...
			UnknownTypecode: -1,
			UnknownBytes:    nil,
			UnifiedAddr:     "u1l8xunezsvhq8fgzfl7404m450nwnd76zshscn6nfys7vyz2ywyh4cc5daaq0c7q2su5lqfh23sp7fkf3kt27ve5948mzpfdvckzaect2jtte308mkwlycj2u0eac077wu70vqcetkxf",
		},
		{
			P2PKHBytes: []byte{
			...
			UnknownTypecode: -1,
			UnknownBytes:    nil,
			UnifiedAddr:     "u1fl5mprj0t9p4jg92hjjy8q5myvwc60c9wv0xachauqpn3c3k4xwzlaueafq27dcg7tzzzaz5jl8tyj93wgs983y0jq0qfhzu6n4r8rakpv5f4gg2lrw4z6pyqqcrcqx04d38yunc6je",
		},
		{
			P2PKHBytes: []byte{
			...
			UnknownTypecode: -1,
			UnknownBytes:    nil,
			UnifiedAddr:     "u1qxqf8ctkxlsdh7xdcgkdtyw4mku7dxma8tsz45xd6ttgs322gdk7kazg3sdn52z7na3tzcrzf7lt3xrdtfp9d4pccderalchvvxk8hghduxrky5guzqlw65fmgp6x7aj4k8v5jkgwuw",
		},
		{
			P2PKHBytes: []byte{
			...
			UnknownTypecode: -1,
			UnknownBytes:    nil,
			UnifiedAddr:     "u1pg2aaph7jp8rpf6yhsza25722sg5fcn3vaca6ze27hqjw7jvvhhuxkpcg0ge9xh6drsgdkda8qjq5chpehkcpxf87rnjryjqwymdheptpvnljqqrjqzjwkc2ma6hcq666kgwfytxwac8eyex6ndgr6ezte66706e3vaqrd25dzvzkc69kw0jgywtd0cmq52q5lkw6uh7hyvzjse8ksx",
		},
		{
			P2PKHBytes: []byte{
			...
			UnknownTypecode: -1,
			UnknownBytes:    nil,
			UnifiedAddr:     "u19mzuf4l37ny393m59v4mxx4t3uyxkh7qpqjdfvlfk9f504cv9w4fpl7cql0kqvssz8jay8mgl8lnrtvg6yzh9pranjj963acc3h2z2qt7007du0lsmdf862dyy40c3wmt0kq35k5z836tfljgzsqtdsccchayfjpygqzkx24l77ga3ngfgskqddyepz8we7ny4ggmt7q48cgvgu57mz",
		},
		{
			P2PKHBytes: []byte{
			...
			UnknownTypecode: -1,
			UnknownBytes:    nil,
			UnifiedAddr:     "u1mtxw5nras5glkxz093282sv3n2h8qs7cpxcmmaxj96vtzjzl6rmdaxs4e9es7mxwmd0h3k5wz3ce4ll5g4jz2pn9su4pufq74pxhp4t235n6j7aed3hh8ss7pf3sekf7apsf6vtg84ue5zcq2k9q3xv5yth3q50fu4czdm8sn8q4de3m5k76g2vwwyjsf50hqfxgmwxqxu0rsy22ktw",
		},
		{
			P2PKHBytes: nil,
			P2SHBytes:  nil,
			...
			UnknownTypecode: -1,
			UnknownBytes:    nil,
			UnifiedAddr:     "u1ay3aawlldjrmxqnjf5medr5ma6p3acnet464ht8lmwplq5cd3ugytcmlf96rrmtgwldc75x94qn4n8pgen36y8tywlq6yjk7lkf3fa8wzjrav8z2xpxqnrnmjxh8tmz6jhfh425t7f3vy6p4pd3zmqayq49efl2c4xydc0gszg660q9p",
		},
		{
			P2PKHBytes: nil,
			P2SHBytes:  nil,
			...
			UnknownTypecode: -1,
			UnknownBytes:    nil,
			UnifiedAddr:     "u19a4vmx7ysmtavmnaz4d2dgl9pyshexw35rl5ezg5dkkxktg08p42lng7kf9hqtn2fhr63qzyhe8gtnvgtfl9yvne46x6zfzwgedx7c0chnrxty0k5r5qqph8k02zs8e3keul9vj8myju7rvqgjaysa9kt0fucxpzuky6kf0pjgy0a6hx",
		},
		{
			P2PKHBytes: nil,
			P2SHBytes:  nil,
			...
			UnknownTypecode: -1,
			UnknownBytes:    nil,
			UnifiedAddr:     "u13p2teem3xlvy4kwlke24hng5el2z6mn4ftj8xarwn8fy7dqt0flgcfpaxe6sk5cwawwh4tynzu7z2uschaf8tfa3tp2xgt8g4kx5lahhglcjm26jnvw7am6ld33708g0kv35pq83eg6gj82a0aau80enrhywpgr4v4m4vve7tg8vd4hz",
		},
		{
			P2PKHBytes: []byte{
			...
			UnknownTypecode: -1,
			UnknownBytes:    nil,
			UnifiedAddr:     "u1snf9yr883aj2hm8pksp9aymnqdwzy42rpzuffevj35hhxeckays5pcpeq7vy2mtgzlcuc4mnh9443qnuyje0yx6h59angywka4v2ap6kchh2j96ezf9w0c0auyz3wwts2lx5gmk2sk9",
		},
		{
			P2PKHBytes: []byte{
			...
			UnknownTypecode: -1,
			UnknownBytes:    nil,
			UnifiedAddr:     "u1szwcx2zdxalyp7cfqwrptv95rnpyajejs6jmwacz4cgm2g3vzdxl5perhpg3nyhnuplvptdr4g63gupdfj5zal9v35s3e6adqsckv68hyrclan3gxaj6mz8aejzsnhqjyn32jcpnpra",
		},
		{
			P2PKHBytes: []byte{
			...
			UnknownTypecode: -1,
			UnknownBytes:    nil,
			UnifiedAddr:     "u1glq6lzrxc7n7r4c922qht20zmpxyl0asfuldrjcaddagfspxpc3040fdfwdf5crw4j6j6wkx4r038s0w24w7enpyfmmdfu9t9p2amxazgvasms8l03l3j5yhrrfqy6xzue5uggef4p8",
		},
		{
			P2PKHBytes: nil,
			P2SHBytes:  nil,
			...
			UnknownTypecode: -1,
			UnknownBytes:    nil,
			UnifiedAddr:     "u1tqhg04ppjt6vlf2uvkygt07sqzgpclxdpn7j7ydkcr0e8ym68wn592z7uqudktrwn4u3q57flp8hw3d0wd9t0rm0e6m8eys27evfawh6zhha6eulzj86uz89swu7gtk0vcknd3dauhc96twhx20xxsp93dxahqlt7z5p04ldgy2y2lp0",
		},
		{
			P2PKHBytes: nil,
			P2SHBytes:  nil,
			...
			UnknownTypecode: -1,
			UnknownBytes:    nil,
			UnifiedAddr:     "u1zm98xj3ncc79sx8jxhcscptxav0p4wam8mlkf4lp69rhramz7v6fsndwxcd4qtmzkefwcwn5rgd8uztvdrvfqv32jk3xx6wlt7gae9fhs7xh48d3kn9fe92xtcff8hu0zgegmgr95qtxayjylfdct96eg2f2r06drf6sj800mcsns3n0",
		},
		{
			P2PKHBytes: nil,
			P2SHBytes:  nil,
			...
			UnknownTypecode: -1,
			UnknownBytes:    nil,
			UnifiedAddr:     "u1hfgf2s4pghqteculnmq2rcnvyesml74zqfp5yfhxhwewx62q75qhgmwreg5qht7c5vu3fxefunjrarrfhmcuw2z4ndx0qx7u74gkw2n7v0ypvd4mxgzlenvs7lkurdj09zuhz6pmtuzs4m42sx92axuuru4dmgu46a920x5kuye6gxvs",
		},
		{
			P2PKHBytes: []byte{
			...
			UnknownTypecode: -1,
			UnknownBytes:    nil,
			UnifiedAddr:     "u17cfcut587e3kszg8vud0z5a8lj9gyypyvtt5xn4hfc4p3kv4e0jfr2pzzxhywlkhsjldtmkvupwr7mkjvruz8gnxk7a64x777p4l3u7vpm6zsdsx88ef90x5q5sqx57fq8vtj5vk3hx",
		},
		{
			P2PKHBytes: []byte{
			...
			UnknownTypecode: -1,
			UnknownBytes:    nil,
			UnifiedAddr:     "u1z6qgxh0wyw0ptgwwgsr5uv05n3xm3z8yrdr06k7q6fj9ypyjcj2hxwfmktv4a7ejaqphcgkddhsvrs93skzl3frm8e48at6huayg7k67e3c50ykpdnhva2jfh5dfcvy6nvttqwgz5a7",
		},
		{
			P2PKHBytes: []byte{
			...
			UnknownTypecode: -1,
			UnknownBytes:    nil,
			UnifiedAddr:     "u1g6jcyfwqd9yx8pdg4yvf0nsr5j7k5gmx83shh8v0v3w256umheen026x66f4608w2vydyasphgp80j9avq9h56dx73gg2559l5lj707v4458a0ucyhfxcjcccfx9z9upmcf3c6hg9k8",
		},
		{
			P2PKHBytes: []byte{
			...
			UnifiedAddr: "",
		},
		{
			P2PKHBytes: []byte{
			...
			UnifiedAddr: "",
		},
		{
			P2PKHBytes: []byte{
			...
			UnifiedAddr: "",
		},
		{
			P2PKHBytes: nil,
			P2SHBytes:  nil,
			...
			UnifiedAddr: "",
		},
		{
			P2PKHBytes: nil,
			P2SHBytes:  nil,
			...
			UnifiedAddr: "",
		},
		{
			P2PKHBytes: nil,
			P2SHBytes:  nil,
			...
			UnifiedAddr: "",
		},
		{
			P2PKHBytes: []byte{
			...
			UnknownTypecode: -1,
			UnknownBytes:    nil,
			UnifiedAddr:     "u1n9znrl4zyuvds24rcapzglzapqdlax4r8rgkvek0y0xlzfjfvn7zexelrafkchea24w030cr9jqsel7t8lvveaq7m7w4z0khmrlzc6748w9ldlccy02scd5xngtcv2yy4ctnyu9zn5m",
		},
		{
			P2PKHBytes: []byte{
			...
			UnknownTypecode: -1,
			UnknownBytes:    nil,
			UnifiedAddr:     "u19f2knszheph2dt8lrnwqeeq9krnw39pgz8syqv028ghtg7kjz6xvu23suv5hmdmj7e6fjuu6060y34fdw8ccjlp8gsqp0usyhrgw3reqfveet7hh2pqcjafysqqv2l3felj7sl7a7ym",
		},
		{
			P2PKHBytes: []byte{
			...
			UnknownTypecode: -1,
			UnknownBytes:    nil,
			UnifiedAddr:     "u160suxvjkgt22zcp7f9xw5f0axdu7rxdt5ktyexpn4cq70w4at2f74390mns7uksfenrdmcjjzqalyfky6tq05jv8mnamrkyxn9dcxe35z4x6m35cczmjcj55g0fc6a2thz03sjfywxa",
		},
		{
			P2PKHBytes:     nil,
			P2SHBytes:      nil,
			...
			UnknownTypecode: -1,
			UnknownBytes:    nil,
			UnifiedAddr:     "u1ddnjsdcpm36r6aq79n3s68shjweksnmwtdltrh046s8m6xcws9ygyawalxx8n6hg6vegk0wh8zjnafxgh6msppjsljvyt0ynece3lvm0",
		},
		{
			P2PKHBytes:     nil,
			P2SHBytes:      nil,
			...
			UnknownTypecode: -1,
			UnknownBytes:    nil,
			UnifiedAddr:     "u1nztelxna9h7w0vtpd2xjhxt4lpu8s9cmdl8n8vcr7actf2ny45nd07cy8cyuhuvw3axcp545y0ktq9cezuzx84jyhex8dk4tdvwhu4dl",
		},
		{
			P2PKHBytes:     nil,
			P2SHBytes:      nil,
			...
			UnknownTypecode: -1,
			UnknownBytes:    nil,
			UnifiedAddr:     "u1trxzh330wl8wkh92uwv508z0qfx270ruuar8fxeng7arry5d73q9ve6gfud36s9nc4qj3uvn082l9srrfayjhnf20mmunywtvqzgc90c",
		},
		{
			P2PKHBytes: nil,
			P2SHBytes:  nil,
			...
			UnifiedAddr: "",
		},
		{
			P2PKHBytes: nil,
			P2SHBytes:  nil,
			...
			UnifiedAddr: "",
		},
		{
			P2PKHBytes: nil,
			P2SHBytes:  nil,
			...
			UnifiedAddr: "",
		},
		{
			P2PKHBytes: []byte{
			...
			UnifiedAddr: "",
		},
		{
			P2PKHBytes: []byte{
			...
			UnifiedAddr: "",
		},
		{
			P2PKHBytes: []byte{
			...
			UnifiedAddr: "",
		},
		{
			P2PKHBytes:     nil,
			P2SHBytes:      nil,
			...
			UnifiedAddr: "",
		},
		{
			P2PKHBytes:     nil,
			P2SHBytes:      nil,
			...
			UnifiedAddr: "",
		},
		{
			P2PKHBytes:     nil,
			P2SHBytes:      nil,
			...
			UnifiedAddr: "",
		},
		{
			P2PKHBytes:     nil,
			P2SHBytes:      nil,
			...
			UnknownTypecode: -1,
			UnknownBytes:    nil,
			UnifiedAddr:     "u1dqavtnjvu42hlsjw6sc2mxajqlyt03zg8l4luykz9fnchunq74nqxhfp58h5n5xfpyqhheax8thta8lfkjgp8wqwsavc0g4mgu4du02c",
		},
		{
			P2PKHBytes:     nil,
			P2SHBytes:      nil,
			...
			UnknownTypecode: -1,
			UnknownBytes:    nil,
			UnifiedAddr:     "u1q8g29qhrktunc24lud3fgk007u7ya8q5g8vy9awadxtl7wu5vjllr4mmdfwk0zdh8zqxgl93sthzumeanzzkdqmqdft6ryhwtvqyqt3e",
		},
		{
			P2PKHBytes:     nil,
			P2SHBytes:      nil,
			...
			UnknownTypecode: -1,
			UnknownBytes:    nil,
			UnifiedAddr:     "u13j3q8q8f9hx2nx0w9l52dqksy4png7fgm0lqjh8ahn9enyvz5z9xnwzdcdjmpf756s2y88rnyr9px4f4k9w03sl6fr4vwsqcvg8ggfjx",
		},
		{
			P2PKHBytes: []byte{
			...
			UnknownTypecode: -1,
			UnknownBytes:    nil,
			UnifiedAddr:     "u1ukslldhknrzmvpdmn03u03edgfy976w3muurfs9asvh3n9uh9h6sgle6m7yjgf3wafxtvke08u735v4nd3kjqnyulw7cvxh6ke357knyjudgqtes6kcw7y28e6kewr03pjah5mh26na",
		},
		{
			P2PKHBytes: []byte{
			...
			UnknownTypecode: -1,
			UnknownBytes:    nil,
			UnifiedAddr:     "u1a0dnfvgdp4khm5yk79ltkkvp8jjmjykjy38cdue8ktl8askwenl4lzfyu0p7end0guyu6up57wylzns0tpr99wz5z8edh5u0m4yzuusysr3d2xczwkp82atq3vfw45u2yvtau852lnw",
		},
		{
			P2PKHBytes: []byte{
			...
			UnknownTypecode: -1,
			UnknownBytes:    nil,
			UnifiedAddr:     "u1a84vn0qes8q3jhk7zxs2whd2p922far8kztqdapergs5ej8rarn53v5ddnd6t7e3l5efhaefrhkptatnzq565nrpvf7kn2787gdvervmk08azp4qgehaew2zplkxkkyu36l3v7drg2v",
		},
		{
			P2PKHBytes: nil,
			P2SHBytes:  nil,
			...
			UnknownTypecode: -1,
			UnknownBytes:    nil,
			UnifiedAddr:     "u187vrwl4ampyxd5m6aj38n4ndkmj8v6gs97hkt23aps3sn5k89a0gk2smluexgdprcrtm56ezc5c7tjwlrnnl79tjtrxmqd42c5mpyz7g",
		},
		{
			P2PKHBytes: nil,
			P2SHBytes:  nil,
			...
			UnknownTypecode: -1,
			UnknownBytes:    nil,
			UnifiedAddr:     "u1xd83nhheggwe78x3lvcygdl8cmwz3gfxnr02sytkxvfpwdep9dzl7vte48zhkx39s705yqp20rw4l835fhg3ylkde44l7glt3cyps5wk",
		},
		{
			P2PKHBytes: nil,
			P2SHBytes:  nil,
			...
			UnknownTypecode: -1,
			UnknownBytes:    nil,
			UnifiedAddr:     "u1w7x9ttwvk30grems6ae3rhgs6xytrrueaklyc5t509fpux7043fzla70jehhxyn4mg9d3ym095s3wghl9trvvdmu56yn74ajqy38ufjg",
		},
		{
			P2PKHBytes: []byte{
			...
			UnknownTypecode: -1,
			UnknownBytes:    nil,
			UnifiedAddr:     "u1smpx6drvevct3dyrer7esjlct99lf4nxdeltdetyxjdrmtqag7q7mkrd8rxlvj9e5vy0qy24fhvvvrj7agfdgxapefxe72xl8vuu9ds5yfq0p86r3y0jw4suurzjz5s6lzrxkfft4am",
		},
		{
			P2PKHBytes: []byte{
			...
			UnknownTypecode: -1,
			UnknownBytes:    nil,
			UnifiedAddr:     "u1ymxkv9nks7tuzjt265fg8vctdq5nxqw4l0q2xj2ya5dkt660rrzkg032v5duhgeqae6cnh9tzxry4dspv8yvtq5lem9gujysaz64034mavd8p0ejqhnvp2jg34nt24y2c2whclxxk94",
		},
		{
			P2PKHBytes: []byte{
			...
			UnknownTypecode: -1,
			UnknownBytes:    nil,
			UnifiedAddr:     "u14j8rtl62a70skh0nhzv7tasxsa69axm0vlac37ye3mcgfpjk6k9ury7hlmet0grhvhedtfj27xmsygp06pcm932f8sc33u5uwps57d89667kyhwmj8pucp5r8cel2lhuaxmx5ftm2nt",
		},
		{
			P2PKHBytes: []byte{
			...
			UnknownTypecode: -1,
			UnknownBytes:    nil,
			UnifiedAddr:     "u1xjkw3lwwf9crx8cz050gdwfejufzhcusc37ged99w8fyj7tyx3e7hgmauyuv538dak2sepq6wjv4tyyjnhcef02dr682y5dsuzuftsx83lrvfc6dxd0kk260m4p3c9ka96vf3z9u6axvsj47mfd6kszy39e5gma28yg88yp92kxjt8ah0x329j4gxjdfyn0n2wp3urwrxxz6z0ynx82",
		},
		{
			P2PKHBytes: []byte{
			...
			UnknownTypecode: -1,
			UnknownBytes:    nil,
			UnifiedAddr:     "u1p4c4u3uz2vtkedv78d4phjav86exankz0x9wmrmz8q4mxqaf43gwd0qt486jk5jvpvyccc6lyy2vaq3ht8ngnw4vusryxd9erhhl2uy5x6x4huyfdymwxj7dkyyeut8ld36kxwu3v5wjg7jwp9kr8ul7u3xdakfunvmwq0rkv6y4k0ngm2n24x763uurfmrr685welsefyys2xwp8ug",
		},
		{
			P2PKHBytes: []byte{
			...
			UnknownTypecode: -1,
			UnknownBytes:    nil,
			UnifiedAddr:     "u1ap7zakdnuefrgdglr334cw62hnqjkhr65t7tketyym0amkhdvyedpucuyxwu9z2te5vp0jf75jgsm36d7r09h6z3qe5rkgd8y28er6fz8z5rckspevxnx4y9wfk49njpcujh5gle7mfan90m9tt9a2gltyh8hx27cwt7h6u8ndmzhtk8qrq8hjytnakjqm0n658llh4z0277cyl2rcu",
		},
		{
			P2PKHBytes: []byte{
			...
			UnknownTypecode: -1,
			UnknownBytes:    nil,
			UnifiedAddr:     "u1udmzarqn6y9026whk083lm5vs8pv282egeln6xg0n2a3w4klkpn6208h68ntuus7gp54d937u4f724v2xgdx6qeu74j45vxfn822xty2yyx6u0ecakj8r9uu3r2jqafj64w7updkhtq",
		},
		{
			P2PKHBytes: []byte{
			...
			UnknownTypecode: -1,
			UnknownBytes:    nil,
			UnifiedAddr:     "u1fyvdgdehrx3gvjx5f2ez2lkcm0lcrfxg8hksdmg3g8zujfz8xk2kyhu4dafs99y96sq2t5c3d3zsxhhnlfmj6trmttg5awtwczz8g8xjr7u30hxc4nkyfyefyl4xt3dxdjevsnrkqdg",
		},
		{
			P2PKHBytes: []byte{
			...
			UnknownTypecode: -1,
			UnknownBytes:    nil,
			UnifiedAddr:     "u1kfzux4hf9favh8jmssqa2h04k87advldqz5ze7a8t4un3nkegklhz3ewzk6lmqg0uy7matdway9vn2q8q9rxp0fjwuewpcjtwrwavxjdsfxdvsk5nkx4q35atp0tfepfdsapqkk4en5",
		},
		{
			P2PKHBytes: []byte{
			...
			UnknownTypecode: -1,
			UnknownBytes:    nil,
			UnifiedAddr:     "u1hrwrtyl3m8m2c6vkhu8wng43j5yvwweg37n2qstsqwc9dfw4vhs69m09064522758p44pfz42gu6hydjxua0wt0ge907sgrxkc9mft4gyfjevkhsyl4d8lnzgyd90arhx4t6v20zlfz",
		},
		{
			P2PKHBytes: []byte{
			...
			UnknownTypecode: -1,
			UnknownBytes:    nil,
			UnifiedAddr:     "u1rf4n5f682jspygln8r5pjwh6fmta7xz6n9x868f5wgc9prxsqkrh8jkpmn7wfnag56ml7czw68dv96299ft6s98p05u4jvdx3elyr83jqnzr603vw8yarptpg5pj73zlea0sksuje3r",
		},
		{
			P2PKHBytes: []byte{
			...
			UnknownTypecode: -1,
			UnknownBytes:    nil,
			UnifiedAddr:     "u1l6exm3zmfsr74sqvlwgc0zf7mydwf6z5r79amka84kfwzwef3wxs0yupl2lwhws85vdmqet3rtz795gpnm4h0jjfv4hanwqta0ezlxqe4p578a4aq09s93xhhtf3xhtrlh575qrsf5g",
		},
	}
	return testVectors
}
